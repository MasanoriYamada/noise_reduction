...
Matrix::Matrix(int i, int j)
{
	...
	//  配列のメモリ領域を動的に確保
	p_top = new COMPLEX*[row+1];
	*p_top = new COMPLEX[row*column+1]; 
...
//       行列の定数倍
//--------------------------------------
Matrix operator*(const Matrix &a, COMPLEX b)
{
...
Matrix operator*(COMPLEX b, const Matrix &a)
{
	Matrix r(a.row, a.column);
	for(int i=1; i<=a.row; i++){
		for(int j=1; j<=a.column; j++){
...
bool Matrix::operator==(const Matrix &a)
{
	...
	for(int i=1; i<=a.row; i++){
	for(int j=1; j<=a.column; j++){
		bool tmp=p_top[i][j] == a.p_top[i][j];
		flag=flag&&tmp;					//tmpに１度でもfalseが入るとfalseを返す
	}
	}
	return flag;
}
